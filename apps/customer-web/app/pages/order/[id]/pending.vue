...
const { data: order, pending } = await useAsyncData(`order-${route.params.id}`, () =>
  api.getOrder(route.params.id as string)
);

useHead(() => ({
  title: order.value ? `Order ${order.value.orderId}` : "Pending order"
}));
